...
def get_nota(order):
    ...
    if row is not None:
        ...
        resp = requests.put(f'{url_fulfillment}/invoiced', json=data, headers=BAGY_HEADERS_API)
        concur.close()
        return (data)
    else:
        qr = (f"select PEDES_COD from TCOM_PEDSAI B where B.PEDS_EXT_COD = 'D{order_code}';")
        concur.execute(qr)
        row = concur.fetchone()
        concur.close()
        con.close()
