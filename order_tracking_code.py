...
def get_tracking_code(order):
    ...
    if order["shipping"]["carrier_id"] == 12341:
        ...
        response = requests.put(
            f"{BAGY_ORDERS_API_BASE_URL}/{order['id']}/fulfillment/shipped",
            json=envio,
    ...
    if row is not None:
        ...
        try:
            ...
            return (
                f"Falha ao buscar tracking do pedido {order['code']} na Braspress: {e}"
            )
    ...
    if envio:
        resp = requests.put(
            BAGY_ORDERS_API_BASE_URL + f"/{order['id']}/fulfillment/shipped",
            json=envio,
            headers=BAGY_HEADERS_API,
        )
        if resp.status_code == 200:
            ...
            return (
                f"Falha ao atualizar pedido {order['code']} na Bagy: {resp.status_code}"
            )
    ...
    if row is not None and str(row[0]) != order["fulfillment"]["nfe_number"]:
        ...
        resp = requests.put(
            f'{BAGY_ORDERS_API_BASE_URL}/{order["id"]}/fulfillment/invoiced',
            json=nfe_data,
            headers=BAGY_HEADERS_API,
        )
        if resp.status_code == 200:
            return (
                f"Nota fiscal do pedido {order['code']} atualizada com sucesso na Bagy!"
            )
        else:
